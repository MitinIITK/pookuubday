...
let words = ["Minuu", "😊"];
let index = 0;
let minuuElement = document.getElementById("minuu");

function changeText() {
    minuuElement.style.opacity = 0; // Fade out
    setTimeout(() => {
        minuuElement.innerHTML = words[index]; 
        minuuElement.style.opacity = 1; // Fade in
        index = (index + 1) % words.length;
    }, 300); // Delay to allow fade-out effect
}

setInterval(changeText, 1000); // Change every 1 second
...
function checkScreenSize() {
    if (window.innerWidth <= 450) {
        document.querySelector('.part1111111').style.display = 'block';
        document.querySelector('.maingamaeqn').style.display = 'none';
    } else {
        document.querySelector('.part1111111').style.display = 'none';
        document.querySelector('.maingamaeqn').style.display = 'block';
    }
}
...
function checkPassword() {
    ...
    const correctPassword = "1292"; // Set your password here
